...
// the heros in the "allHeroContainers" variable you just made
...
let allHeroContainers = getElementsByClassName("hero_container");

function removeIds(){
    for(let i = 0; i < allHeroContainers.length; i++){
        allHeroContainers[i].removeAttribute("id")
    }
}

function highlighthero(event_object){
    event_object = removeIds(event.currentTarget)

}

for(let i = 0; i < allHeroContainers.length; i++){
    allHeroContainers[i].addEventListener("click", highlighthero());
}



//Here starts hero.js, step 4.

let customName = document.getElementById('inputName');
let submitButton = document.getElementById('submitButton');

submitButton.addEventListener('click', submitForm(customName))